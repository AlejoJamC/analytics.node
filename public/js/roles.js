            ...
            $.each(iterador, function (i, abreviData) {
                for (i = 0; i < data.rows.length; i++) {
                    trHTML += '<tr class="odd gradeX">' +
                        ...
                        '<a href="/settings/roles/' + data.rows[i][0].toLowerCase() + '" class="btn btn-outline btn-circle btn-sm purple">' +
...
function EliminarRolModal(idrol) {
    // Validar si envio vacio
    if (idrol === '' || typeof idrol === 'undefined' || idrol === null) {
        swal({
            title: "Datos Incompletos",
            message: "",
            type: "warning",
    ...
    swal({
        title: "Se va eliminar el rol id: " + idrol,
