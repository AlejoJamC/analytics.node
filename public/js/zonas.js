            ...
            $.each(iterador, function (i, abreviData) {
                for (i = 0; i < data.rows.length; i++) {
                    trHTML += '<tr class="odd gradeX">' +
                        ...
                        '<a href="/settings/zones/'+ data.rows[i][0].toString().toLowerCase() + '" class="btn btn-outline btn-circle btn-sm purple">' +
...
function GuardarModalNuevo() {
    ...
    $.ajax({
        method: "GET",
        url: "/settings/zones/check/" + $idrol + "/ajax"
    })
        .done(function (data) {
            console.log(data);
                ...
                $.ajax({
                    ...
                    .done(function (data) {
                        swal({
                                ...
                                cancelButtonText: "Agregar otra zona",
                                closeOnConfirm: true
                            },
                            function (isConfirm) {
                                if (isConfirm) {
                                    window.location.href = "/settings/zones";
                                } else {
                                    window.location.href = "/settings/zones/new";
                                }
                            });
                    });
                return evitarSubmit;
...
function EliminarModal(id) {
    ...
    swal({
        title: "Se va eliminar la zona id: " + id,
